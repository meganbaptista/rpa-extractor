...
- **Root cause 3 (placeholder masquerading as a value):** on the same corrupt PDF, Opus couldn't read the buyer name on page 1 and returned the literal string `"<UNKNOWN>"` for `buyer_names`/`buyer_1` instead of an empty string. That is doubly bad: (a) it's non-empty so the `missingCritical` safety net skipped it, and (b) `<UNKNOWN>` parses as an unknown HTML tag in the review UI and renders as NOTHING, so a missed field looked like an ordinary blank. Fix: new **`lib/sanitize.js`** `scrubPlaceholders()` converts placeholder sentinels (`<...>`, `UNKNOWN`, `N/A`, `TBD`, `see addendum`, `unreadable`, ...) back to `""`. Called in BOTH functions twice — once BEFORE the missingCritical check (so a fake-filled critical field actually re-escalates) and once BEFORE storing (so nothing fake reaches the UI/Zapier even if Opus re-introduces it). Exact-match only (never substrings) so real values like "NABR Realty" / "TBD Holdings LLC" / "None of the above" are untouched. `buyer_names` description in `lib/rpa-fields.js` also gained an explicit "never return a placeholder, return empty string" rule as defense-in-depth.
- **Still UI-side (not yet done):** `index.html` injects field values as HTML without escaping, which is why `<UNKNOWN>` rendered invisible. With the scrub in place this no longer bites, but escaping rendered values would be a good hardening follow-up.
