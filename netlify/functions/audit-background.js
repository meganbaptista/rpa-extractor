...
exports.handler = async function (event) {
  ...
  if (!parsedBody.ok) {
    console.error(`[audit-background] invalid request body - ${parsedBody.diagnostic}`);
  ...
  if (parsedBody.note) console.warn(`[audit-background] ${parsedBody.note}`);
...
// thrown network errors). Signature-audit judgment is liability-sensitive, so effort
// stays 'high' (the shared default). The truncation guard's custom message is passed
// through as maxTokensError; usage is logged inside the shared call, before the guard.
function callClaude(prompt, pdfBase64) {
  return callClaudeShared({
    ...
    maxTokens: 32000,
    maxTokensError: 'Audit response hit max_tokens -- output was truncated before completion. '
