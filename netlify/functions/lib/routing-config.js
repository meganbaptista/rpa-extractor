...
const NO_TAG_RULES = [
  ...
  'LOAN / LIEN PAYOFF to be cleared at closing -> NO_TAG: escrow/title clears it from the sale proceeds and our team takes no action. This covers a SOLAR loan payoff (a "paid in full" / "paid off" / payoff / "Payment Received" letter, e.g. Solar Mosaic, Solar Servicing, or Sunrun) AND a CONCIERGE / seller bridge/renovation-loan payoff statement (e.g. Compass Concierge, or Notable / Concierge Capital, noreply@notablefi.com). It holds even when the newest message is just the agent forwarding the letter to escrow, and even though it carries a payoff attachment and an "Action Required"-style subject. Do NOT route it by buyer/seller side, and do NOT route it to Megan merely because she is cc\'d or the thread says "Megan\'s team". EXCEPTION: if the email specifically DIRECTS the ask to Megan (addresses Megan by name, or asks Megan / our team to handle the payoff) -> Megan.',
...
const ROUTING_NOTES = [
  ...
  'LOAN / LIEN PAYOFFS (solar loan payoffs such as Solar Mosaic / Solar Servicing / Sunrun, and concierge / bridge loan payoffs such as Compass Concierge / Notable / Concierge Capital) are NO_TAG unless the email directs the ask to Megan — see the "LOAN / LIEN PAYOFF" no-tag rule.',
