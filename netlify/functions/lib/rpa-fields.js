...
const FIELDS = {
      ...
      county: { type: "string", description: "County name only, WITHOUT the word 'County' — e.g. 'Los Angeles', never 'Los Angeles County'. Proper case. The property profile header appends 'County' after the name (e.g. '...90291, Los Angeles County'); strip it." },
      ...
      buyer_1: {
        type: "string",
        description: `First buyer, split out for downstream systems that need one buyer per field. ONE HUMAN OR ONE ENTITY PER SLOT. Source: the RPA paragraph 1A "THIS IS AN OFFER FROM" buyer entry — which can WRAP onto a second/continuation line when there are 3+ buyers; include every name across ALL lines (and paragraph 32B for the full entity name). These names are already in natural First-Last order — do NOT rebuild them like the courthouse Last-First seller fields.
...
• THREE OR MORE buyers often WRAP onto a second line on 1A — e.g. 'Lauri Grenner, Devyn Grenner' on the first line and 'Brian Smilowitz' just below it. Treat every name across BOTH lines as its own buyer: buyer_1 'Lauri Grenner', buyer_2 'Devyn Grenner', buyer_3 'Brian Smilowitz'. Never ignore the continuation line.
...
If no Property Profile is provided, fall back to MLS or RPA paragraph 33 signatures, applying the same one-human-per-slot rule to whatever source you're using. seller_1 must NEVER be blank — at minimum the primary seller goes here.`
      ...
      buyer_agent_brokerage_name: { type: "string", description: "Buyer's brokerage firm name from the 'Buyer's Brokerage Firm' line in subsection A on the LAST PAGE of the RPA. CRITICAL: subsection B 'Seller's Brokerage Firm' is directly below on the same page and has its own brokerage name (the listing brokerage) — that is NOT the buyer's brokerage. The buyer's brokerage name appears on the line that begins 'A. Buyer's Brokerage Firm'. Read the firm name printed after that label, e.g. a line reading 'A. Buyer's Brokerage Firm <FIRM_NAME>' returns the <FIRM_NAME> text. BLANK / UNREADABLE RULE: if the 'A. Buyer's Brokerage Firm' line is blank, the page is illegible, or you cannot clearly read an actual firm name on the page, return an EMPTY STRING. Never guess a brokerage name and never echo the placeholder text from this instruction." },
      buyer_agent_brokerage_dre: { type: "string", description: "Brokerage DRE on the SAME LINE as the buyer brokerage firm name in subsection A (NOT the agent's line). DO NOT pull from subsection B (Seller's Brokerage Firm) — that is the seller's brokerage DRE, a different number. The buyer brokerage DRE is on the same line as the firm named in subsection A, e.g. 'A. Buyer's Brokerage Firm <FIRM_NAME>   DRE Lic. # <########>' returns the <########> digits. BLANK / UNREADABLE RULE: if the firm line has no DRE number, the page is illegible, or you cannot clearly read an actual license number, return an EMPTY STRING. Never guess a DRE and never echo the placeholder text from this instruction." },
